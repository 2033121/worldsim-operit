...
1. 段落要短：每段最多 2~3 行，动作、对话、反应**各自独立成段**。
2. 动词优先：多用"推门、攥紧、蹲下、抬头、转身"这类动作动词推进画面，少用形容词堆叠。
3. 环境信息化：环境描写不超过全章 1/10，且必须带信息或异常感（"灯管闪了两下"→ 暗示异常），禁止纯风景抒情。
4. 砍掉 80% 铺垫：场景切换直接"切"，背景信息在对话和动作里带出来。
5. 对话占比要高：每章对话至少占 40% 篇幅，对话就是情节。
章节骨架（每章按这个四拍走）：
