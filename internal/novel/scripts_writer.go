...
2. 对话独立成段：每句对白=一个段落（可带一个动作提示），禁止把两人对话挤进同一段；对话占比必须≥50%，对话就是情节推进器（"问什么了？""灵田几亩，种什么，谁在料理。"）。
3. 动词优先：多用"踹、拍、攥、甩、砸、退、瞪、蹲、摸"这类动作动词推进，禁止形容词堆叠和文青式长句（"惨白的月光"→"月光惨白"；"他默默地望着远方"→"他望着田埂那头，半晌没动"）。
4. 开场即冲突：第一段直接进画面或砸冲突（对峙/逼债/发现异常/有人上门），环境铺垫最多2句且必须带信息（"雨丝连绵，打在祠堂瓦片上噼啪作响"），禁止超过2句的纯风景铺垫。
5. 信息具体化：数字、等级、物件、金额、地名都要具体（"练气三层""三千灵石""苍蛟剑""白鹤坊市"），禁止"很久以前""数量很多""某种东西"这种模糊词。
